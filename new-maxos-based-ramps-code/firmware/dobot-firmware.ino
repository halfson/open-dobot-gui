...
Additional Authors: Mike Ferguson www.mikeahferguson.com (May 15 2016)
...
https://www.youtube.com/watch?v=kFweLvioovo Arduino IDE (near the end, it briefly discusses how fast serial data can be sent on the arduino)
https://www.youtube.com/watch?v=faCVhp7gm-g Arduino Timers and Counters
https://www.youtube.com/watch?v=twaeLkE6eS8 lecture describing serial communication basics, useful lectures afterwards too
https://www.youtube.com/watch?v=PGf98rGBEJk Queues in serial communication
https://www.youtube.com/watch?v=Mr7rkEX0SxM Serial communication examples (part 1 of 3 part lecture, discusses queues)
...
https://www.youtube.com/watch?v=BUubcDvvOPo Arduino Interrupt Processing Part I (of 2 part series) [This explains the use of the volatile keyword, summarized below]

A brief explanation of the volatile keyword: If a variable is shared between a normal function (e.g. the loop() function) and an interrupt function (aka ISR aka Interrupt Service Routine), 
it should be declared volatile. This is because the ISR interrupts the normal function and can change the shared variable in the normal function. Upon finished execution of the ISR, 
the program returns to the normal function and may use the modified shared variable (that was changed by the ISR). If the variable was not declared volatile, then the value of the variable
as set by the ISR will not be used (because the normal function has not modified the memory to update the value of the shared variable). If the shared variable was identified as volatile,
upon return to the normal function, the shared variable is updated to the value set by the ISR. So, I'm not sure if this is right, but it is only necessary to declare a shared variable
voaltile if it will be modified by the ISR? and not simply used? though I imagine it's probably best practice to declare the variable volatile anyways, even if it's only read?
...
#define J3_STEP_PIN         60
#define J3_DIR_PIN          61
