...
class DobotGUIApp(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.dobot = Dobot('COM9', 9600)
        self.dobot.Open()
        time.sleep(1)
        successes = 0
        i = 0
        while True:
            ret = self.dobot.isReady()
            if ret[0] and ret[1]:
                successes += 1
            if successes > 10:
                print ("Dobot ready!")
    ...
    def move_stepper_motors_using_line_step_sequence(self):
        ...
        for i in range(1, (len(self.stepperSequenceToDrawALine)-1), 6):#note the -1 in the range limit, which accounts for the 'e' end character. Also note that it increments by 6 and starts at index 1 (2nd list item), after 's'
            ...
            # copied from the Steps function in Dobot.py
            #joint1 = base
            #joint2 = upper arm
            #joint3 = lower arm
            '''
            ...
            @param ticks - number of ticks to run the command for (Mike's note: number of steps to take = ticks * 2, dunno if this is what was intended )
            ...
            '''
            self.dobot.Steps(self.stepperSequenceToDrawALine[i+3], self.stepperSequenceToDrawALine[i+4], self.stepperSequenceToDrawALine[i+5],
                             2,
                             self.stepperSequenceToDrawALine[i], self.stepperSequenceToDrawALine[i+1], self.stepperSequenceToDrawALine[i+2],
                             False)
